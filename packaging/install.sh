...
case "$os" in
    ...
    *) fail "Unsupported operating system: $os. repo-scanner currently ships binaries for Linux and macOS (x86_64) - see specs/020-cli-packaging/research.md §9." ;;
esac

arch="$(uname -m)"
case "$arch" in
    x86_64|amd64) target_arch="x86_64" ;;
    *) fail "Unsupported architecture: $arch. repo-scanner currently only ships x86_64 binaries - see specs/020-cli-packaging/research.md §9." ;;
esac
...
[ -n "$download_url" ] || fail "No release asset found for ${target_os}-${target_arch}. This build of repo-scanner does not (yet) support this platform - see specs/020-cli-packaging/research.md §9."
...
echo "repo-scanner $installed_version installed to $INSTALL_DIR/$BINARY_NAME"
echo "Verify with: repo-scanner --version"
