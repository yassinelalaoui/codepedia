...
1. Run `codepedia index /path/to/sample-repo`.
...
2. Run `codepedia index /path/to/sample-repo`.
...
5. Run `codepedia index /path/to/sample-repo` again.
...
1. Run `codepedia index /path/does/not/exist`.
2. Confirm the command exits non-zero immediately, with a message naming
   ...
   `~/.codepedia/repos/<state-id>/` directory is created as a result
...
1. With the sample repository already indexed (golden-path scenario
   ...
   `codepedia serve /path/to/sample-repo`.
...
3. Run `codepedia config --llm-model <a-different-installed-model>`.
4. Confirm it saves successfully and that a subsequent `codepedia
   config --show` reflects the new value.
5. Run `codepedia config --embedding-model
...
7. Run `codepedia index /path/to/sample-repo` again.
...
1. Run `codepedia scan /path/to/sample-repo`.
2. Confirm the output still matches
   `specs/001-local-codepedia/contracts/cli.md` exactly (a JSON
   document with `root_path`, `generated_at`, `entries`, `summary`),
   unchanged by this feature.

## Expected result

Running one command (`codepedia index`) against a valid local
...
partial work. `codepedia serve` resumes an already-indexed repository
with live updates. `codepedia config` lets a developer choose and
