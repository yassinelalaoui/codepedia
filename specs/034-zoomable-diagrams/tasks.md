...
- [X] T017 [US1] Implement drag-to-pan in `frontend/src/lib/diagramViewport.ts` — `pointerdown`/`pointermove`/`pointerup` translating by the frame-to-frame delta, tracked on `window` only (a move inside the viewport bubbles to both, and handling it twice worked only by accident). **`setPointerCapture` must NOT be used at all** — this task originally called for it behind a `typeof` guard, and that shipped a bug: capture retargets the following `click` to the capturing element, so Mermaid's `<a>` node links stopped navigating entirely. jsdom does not define `setPointerCapture`, so the guard skipped the path and no test could see it. Found in Chrome; now pinned by a source-level regression guard. Makes T014's pan case pass
...
- [X] T039 Execute the browser pass on a regenerated wiki. Done automatically via headless Chrome over CDP (quickstart § 3a): **17/17 on both click-target surfaces**. This found two real bugs the whole jsdom suite passed over - `mermaid.run()` missing `querySelector` (no diagram rendered at all) and `setPointerCapture` swallowing every node-link click. Steps needing human eyes remain in § 3b: dark mode, reduced motion, and reading a genuinely dense diagram. **Required, not optional**: SC-003 and SC-004 cannot be satisfied by any headless test, because whether Mermaid's own anchor still fires under a transformed ancestor is not something jsdom can answer
