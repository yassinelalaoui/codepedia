...
- `doc_generator` — `doc-manifest.sqlite`: the page manifest (what was
  generated, its content hash, its links) used to compute incremental
  regeneration impact, plus caches of model output that is expensive to
  reproduce. (033, 039) `doc_feature_plans` keeps the planner's one answer per
  repository, keyed by a hash of the module keys, the entry-point keys and the
  grouping it named (`GROUPING_VERSION` plus each group's members). A regrouped
  repository therefore never gets old titles on new groups, and an unchanged
  one never asks twice. `doc_page_aliases` keeps every address a moved feature
  page used to have. (038) `doc_overview_narratives` keeps one raw Overview narrative
  reply per repository, keyed by a hash of the exact prompt and stored with a
  repository fingerprint (a hash of every file's path and content hash). The
  fingerprint is what lets a fallback tell "the same repository, asked an
  earlier question" (shown as is) from "an earlier version of the repository"
  (shown with a caveat). The reply is stored raw and re-grounded on every
  load, so a changed rule or a removed symbol takes effect with no new call.
  The column was added in place to existing tables (`ADDED_COLUMNS`).
